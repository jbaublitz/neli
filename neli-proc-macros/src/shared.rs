...
use syn::{
    ...
    ItemStruct, LifetimeParam, Meta, MetaNameValue, Path, PathArguments, PathSegment, Token,
...
pub fn process_lifetime(generics: &mut Generics) -> LifetimeParam {
    if let Some(GenericParam::Lifetime(lt)) = generics.params.first() {
        lt.clone()
    } else {
        let mut punc = Punctuated::new();
        let lt = parse::<LifetimeParam>(TokenStream::from(quote! {
            'lifetime
        }))
